...
import { ENV, SoundFileOptions, FrenchPronunciationApi } from '../config/env'
...
export function usePlayWordAudio() {
  ...
  function playAudio(word: string, handle: boolean = true, onEnd?: () => void, onPlay?: () => void, audioPath?: string) {
    const wordAudio = getCachedWordAudio()
    ...
    let url = ''
    if (audioPath) {
      // 词库自带真人发音（本地 mp3），优先使用
      url = ENV.RESOURCE_URL + (audioPath.startsWith('/') ? audioPath : `/${audioPath}`)
    } else {
      // 无本地音频，走在线发音 API 兜底（如自定义词典 / 即时查词，法语）
      url = `${FrenchPronunciationApi}${encodeURIComponent(word)}`
    }
    let onended = () => {
...
export function useTTsPlayAudio() {
  ...
  function play(text: string, options: TTsPlayOptions = {}) {
    ...
    getVoicesAsync().then((voices: any[]) => {
      ...
      // 回退：优先找法语音色，否则取第一个法语声色
      let voiceList = voices.filter(v => v.lang === 'fr-FR')
      if (voiceList && voiceList.length) {
        msg.voice = voiceList.find(v => v.name.includes('FR') || v.name.includes('French')) ?? voiceList[0]
      }
      speechSynthesis.speak(msg)
    })
  }
