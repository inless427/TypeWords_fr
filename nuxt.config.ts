...
const siteOrigin = (process.env.ORIGIN || 'https://type.ennuyez-vous.xyz').replace(/\/$/, '')
...
export default defineNuxtConfig({
  ...
  app: {
    ...
    // keepalive: true,
    head: {
      title: 'Type Words — 免费法语单词练习 | Free French Typing Practice', // default fallback title
      htmlAttrs: {
        lang: 'zh-CN',
      },
  ...
  runtimeConfig: {
    public: {
      ...
      origin: process.env.ORIGIN || 'https://type.ennuyez-vous.xyz',
      host: process.env.HOST || 'type.ennuyez-vous.xyz',
